...
from ev_couplings_v4 import EVcouplings, ALPHABET_PROTEIN_NOGAP
...
class Deimmunization(object):
    ...
    def __init__(self, args):
        ...
        # ev_couplings.to_zero_sum_gauge()  # TODO: is there an equivalent?
        ...
        self.set_possible_mutations(str(source_record.seq).upper())
    ...
    def set_possible_mutations(self, source_seq):
        # set to wildtype residues
        self.possible_mutations = [set(res) for res in self.wildtype.sequence]

        # mapping from wildtype to source
        aln_wt, aln_source, _, _, _ = pairwise2.align.globalds(
            self.wildtype.sequence, source_seq.upper(),
            matlist.blosum62, -11, -1
        )[0]
        mapping = defaultdict(lambda: -1)
        wt_res_num, source_res_num = 0, 0
        for wt_res, source_res in zip(aln_wt, aln_source):
            if wt_res != '-' and source_res != '-':
                mapping[wt_res_num] = source_res_num
                wt_res_num += 1
                source_res_num += 1
            elif wt_res == '-':
                source_res_num += 1
