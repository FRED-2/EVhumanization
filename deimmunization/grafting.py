...
class Grafting(object):
    """Abstract class for grafting a subset of residues of the sequence to be humanized onto the human template sequence.

    Attributes:
        sequence -- SeqRecord of the sequence to be humanized
        template -- SeqRecord of the human sequence serving as a template in the humanization process
        mapping -- mapping of sequence to template as dictionary: key = residue number of sequence, value = residue number of template
                   (residues not zero-based numbered, i.e. starting from 1)
        residues_to_graft -- list of residues of the sequence to be grafted onto the template (needs to be set in inheriting class)
        humanized_seq -- SeqRecord of the humanized sequence, i.e. resulting sequence by grafting residues_to_graft onto template
    """
    ...
    def find_residues_to_graft():
        """Find residues of sequence to graft onto template and fill attribute residues_to_graft accordingly."""
        pass
    ...
    def map_sequence_to_template(self, gap_open=-11, gap_extend=-1):
        ...
        Args:
            gap_open: penalty for introducing a gap in alignment calculation (default: -11)
            gap_extend: penalty for extending a gapped section in alignment calculation (default: -1)
        """
        aln_sequence,aln_template,_,_,_  = pairwise2.align.globalds(self.sequence.seq.upper(), self.template.seq.upper(),
    ...
    def graft(self):
        """Graft residues_to_graft onto template and save SeqRecord of the resulting humanized sequence."""
        ...
        self.humanized_seq = SeqRecord(seq=Seq(''.join(mod_seq), SingleLetterAlphabet()),
    ...
    def find_residues_to_graft(self):
        ...
        for b, e in loop_regions:
            for i in xrange(b[0], e[0]+1):
                ...
                if i == e[0]:
                    search_alpha = CDRGrafting._ALPHABET[:operator.indexOf(CDRGrafting._ALPHABET,e[1])+1] if e[1] != '' else []
...
class SDRGrafting(Grafting):
    ...
    def make_contact_map_wrapper(pdb_id, chains):
        ...
        pair_dist_lists = make_contact_map(pdb_id, list(chains), pdb_file_name=pdb_file_name, use_sifts_mapping=True)
