...
class Grafting(object):
    ...
    def graft(self):
        ...
        residues_to_graft_mapped = [self.mapping[res_num]
                                    for res_num in self.residues_to_graft]
        print >> stderr, 'Template indices of residues grafted:'
        print ','.join(map(lambda x: str(x-1), [x for x in residues_to_graft_mapped
                                                if not x == -1]))
    ...
    def to_file(self, output):
        ...
        print >> stderr, 'Humanized sequence written to %s' % output
